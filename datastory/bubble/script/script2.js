...
function x(d) {  return d.years; }
function y(d) { return d.emissions; }
function radius(d) { return d.emissions; }
function color(d) { return d.name; }
function key(d) { return d.name; }

// Chart dimensions.
var margin = {top: 19.5, right: 19.5, bottom: 19.5, left: 139.5},
...
var xScale = d3.scale.linear().domain([1950,2020]).range([0, width]),
    yScale = d3.scale.linear().domain([yMin,yMax]).range([height, 0]),
    radiusScale = d3.scale.sqrt().domain([0, 12454710]).range([0, 100]),
...
var label = svg.append("text")
    ...
    // Load the data.
    d3.json("data/extract_data.php", function(nations) {
console.log(nations);
      ...
      function interpolateData(year) {
        return nations.map(function(d) {
          return {
            name: d.name,
            population: interpolateValues(d.emissions, year),
