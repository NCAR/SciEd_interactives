 ...
 *  
 * TODO: handle close but not quite case
 */
	...
	$.getJSON($jsonFile, function json_process(data) {
		...
		$.each(data.SORT.DRAGGABLE, function create_draggable(i) {
			// only if had data
				...
				var draggableCloseResponseTitle = this.CloseResponseTitle;
				var draggableCloseResponseBody = this.CloseResponseBody;
				...
				html += '<div class="closeResponse"><h1>' + draggableCloseResponseTitle + '</h1><span class="textBody">' + draggableCloseResponseBody + '</span></div>';
		...
		$dropzone_lower.droppable({
			drop : function droppable_lower(event, ui) {
				if(ui.draggable.is('.wrong-lower')){
					// close but still wrong
					$('#wrongAudio').get(0).play();
					closeAnswer(ui.draggable.find('div.closeResponse'));
		...
		/*
		 * make the middlezone droppable and handle interactions
		 */
		...
		/*
		 * answer is close, but still wrong, don't deduct score, but give notice
		 */
		function closeAnswer($response) {
			// populate closefeedback text
			$('div.response').empty();
			$response.clone().appendTo('div.response');
			$("div.response div.closeResponse").css('display', 'inline').effect("bounce", {
				times : 3
			}, 300);
		}
